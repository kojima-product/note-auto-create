...
**重要: 「===ここから有料===」の位置は記事全体の20〜35%の位置に配置してください。**
例えば4000文字の記事なら、最初の800〜1400文字が無料プレビューです。
短すぎる導入（数行だけ）は絶対にNGです。読者が「続きを読みたい！」と思える十分なボリュームが必要です。
...
6. 以下の構成で書いてください:
   - 導入（読者の興味を引くフック、背景説明、ニュースの概要 — 記事全体の20〜35%のボリュームで）
   - ===ここから有料===（この行を必ず入れてください。導入が短すぎないよう注意！）
